...
export default function Navbar() {
  ...
  return (
    <nav style={{ background: 'var(--bg-panel)', borderBottom: '1px solid #2c3e50', padding: '15px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
      
      <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '15px' }}>
        <span style={{ fontSize: '1.8rem' }}>🏮</span>
        <h2 style={{ margin: 0, color: 'var(--lantern-gold)', letterSpacing: '2px', textTransform: 'uppercase', fontSize: '1.2rem' }}>The Lantern Library</h2>
      </Link>

      <div style={{ display: 'flex', alignItems: 'center', gap: '25px' }}>
        <Link to="/messages" style={{ textDecoration: 'none', color: 'var(--text-main)', fontWeight: 'bold', fontSize: '1rem', transition: 'color 0.2s' }}>💬 Whispers</Link>
        <Link to="/community" style={{ textDecoration: 'none', color: 'var(--text-main)', fontWeight: 'bold', fontSize: '1rem', transition: 'color 0.2s' }}>🗣️ Lounge</Link>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        <button onClick={() => navigate('/write')} style={{ background: 'var(--lantern-gold)', color: 'var(--bg-deep)', border: 'none', padding: '8px 20px', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ✏️ Publish
        </button>
        
        <div style={{ background: 'transparent', border: '1px solid var(--lantern-gold)', color: 'var(--lantern-gold)', padding: '6px 15px', borderRadius: '20px', fontWeight: 'bold', fontSize: '0.9rem' }}>
          STREAK 🔥 2
        </div>

        {/* 🔔 NOTIFICATION BELL */}
        <div style={{ position: 'relative' }} ref={notifRef}>
          <button onClick={() => setShowNotifDropdown(!showNotifDropdown)} style={{ background: 'transparent', border: 'none', fontSize: '1.5rem', cursor: 'pointer', position: 'relative', padding: '5px' }}>
            🔔
            {unreadCount > 0 && (
          ...
          {showNotifDropdown && (
             <div style={{ position: 'absolute', top: '100%', right: '0', marginTop: '15px', width: '320px', background: 'var(--bg-panel)', border: '1px solid var(--lantern-gold)', borderRadius: '8px', zIndex: 1000, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }} ref={profileRef}>
          <img 
            ...
            alt="Profile" 
            style={{ width: '45px', height: '45px', borderRadius: '50%', border: '2px solid var(--lantern-gold)', background: '#ecf0f1', cursor: 'pointer' }} 
          />
          ...
          {showProfileMenu && (
            <div style={{ position: 'absolute', top: '100%', right: '0', marginTop: '15px', width: '200px', background: 'var(--bg-panel)', border: '1px solid #34495e', borderRadius: '8px', zIndex: 1000, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
              <div 
                ...
                style={{ padding: '15px', color: 'var(--text-main)', cursor: 'pointer', borderBottom: '1px solid #2c3e50', fontWeight: 'bold' }}
              >
                📚 My Archives
              </div>
              <div 
              ...
              <div 
                ...
                style={{ padding: '15px', color: '#e74c3c', cursor: 'pointer', fontWeight: 'bold', background: 'var(--bg-deep)' }}
              >
                🚪 Log Out
              </div>
            </div>
