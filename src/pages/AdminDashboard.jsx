...
export default function AdminDashboard() {
    ...
    const fetchDashboardData = async () => {
        ...
        // 1. Fetch NGOs Safely
        try {
            const ngoRes = await fetch(`${API_URL}/ngos`);
            if (ngoRes.ok) {
                const ngoData = await ngoRes.json();
                if (isMounted.current) setNgos(Array.isArray(ngoData) ? ngoData : []);
            }
        } catch (err) {
            console.error("Failed to load NGOs:", err);
        }

        // 2. Fetch Campaigns Safely (If this fails, NGOs still show up!)
        try {
            const campRes = await fetch(`${API_URL}/campaigns`);
            if (campRes.ok) {
                const campData = await campRes.json();
                if (isMounted.current) setCampaigns(Array.isArray(campData) ? campData : []);
            }
        } catch (err) {
    ...
    const handleApprove = async (id) => {
        try {
            ...
            if (res.ok) {
                ...
                alert("Failed to approve NGO. Check console for errors.");
            }
        } catch (err) {
    ...
    return (
        <div className="animate-fade-in relative max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                ...
                <DashboardCard 
                    ...
                    list={pendingNgos} 
                    renderItem={ngo => (
                        <div className="flex justify-between items-center py-3 border-b border-slate-100 last:border-0" key={ngo._id}>
                            <div>
                            ...
                            <Button onClick={() => setReviewNgo(ngo)} variant="secondary" className="px-3 py-1.5 text-xs bg-white text-[#0B2948] border-[#0B2948]">
                                Review Data
                            </Button>
                        </div>
                    )}
                ...
                <div className="fixed inset-0 bg-[#0B2948]/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-fade-in">
                        ...
                        <div className="p-6 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
                            <Button onClick={() => setReviewNgo(null)} variant="secondary" className="px-6 text-slate-600 border-slate-300 bg-white">
                                Cancel
                            </Button>
                            <Button 
                                onClick={() => { 
                                ...
                                variant="primary" 
                                className="px-6 bg-green-600 hover:bg-green-700 border-green-700 text-white"
                            >
