  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://lantern-library-backend.onrender.com/api/ai/summon', {
        ...
        body: JSON.stringify({
          characterName, mediaTitle, userMessage,
      ...
      if (response.ok) setMessages(prev => [...prev, { sender: 'ai', text: data.reply }]);
      else setMessages(prev => [...prev, { sender: 'system', text: '❌ Connection lost.' }]);
    } catch (error) { setMessages(prev => [...prev, { sender: 'system', text: '❌ Server error.' }]); } 
        ...
        {!isSummoned ? (
          <div className="chat-tunnel" style={{ justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
            ...
            <form onSubmit={handleSummon} style={{ display: 'flex', flexDirection: 'column', gap: '15px', width: '100%', maxWidth: '400px' }}>
              <input type="text" placeholder="e.g. Paul Atreides, Elizabeth Bennet" value={characterName} onChange={(e) => setCharacterName(e.target.value)} style={{ padding: '15px', borderRadius: '12px', border: '1px solid var(--border-color)', fontSize: '1rem', textAlign: 'center' }} required />
              <button type="submit" style={{ padding: '15px', background: 'var(--lantern-gold)', color: '#fff', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold' }}>Begin Ritual</button>
            </form>
          </div>
        ) : (
          <>
            <div className="chat-tunnel">
              {messages.map((msg, index) => (
                <div key={index} className={`bubble-wrapper ${msg.sender === 'user' ? 'sent' : 'received'}`}>
                  {msg.sender === 'ai' && <strong style={{ marginBottom: '4px', color: 'var(--lantern-gold)', fontSize: '0.8rem', paddingLeft: '5px' }}>{characterName}</strong>}
                  <div className={`chat-bubble ${msg.sender === 'user' ? 'sent' : 'received'}`}>{msg.text}</div>
                </div>
              ))}
            ...
            <form onSubmit={sendMessage} className="chat-input-area">
              <input type="text" placeholder={`Speak to ${characterName}...`} value={input} onChange={(e) => setInput(e.target.value)} style={{ flexGrow: 1, padding: '12px 20px', borderRadius: '25px', outline: 'none' }} disabled={loading} />
              <button type="submit" disabled={loading} style={{ padding: '0 25px', background: 'var(--lantern-gold)', color: '#fff', border: 'none', borderRadius: '25px', cursor: 'pointer', fontWeight: 'bold' }}>Send</button>
            </form>
          </>
        )}
      </div>
